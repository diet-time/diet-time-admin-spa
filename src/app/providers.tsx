...
export function AppProviders({children}:PropsWithChildren){const {i18n}=useTranslation();const rtl=i18n.language==='ar';const theme=useMemo(()=>createDietTimeTheme(rtl?'rtl':'ltr'),[rtl]);const cache=useMemo(()=>createCache({key:rtl?'mui-rtl':'mui',stylisPlugins:rtl?[prefixer,rtlPlugin]:[prefixer]}),[rtl]);document.documentElement.lang=rtl?'ar':'en';document.documentElement.dir=rtl?'rtl':'ltr';return <CacheProvider value={cache}><ThemeProvider theme={theme}><CssBaseline/><QueryClientProvider client={queryClient}>{children}</QueryClientProvider></ThemeProvider></CacheProvider>}
