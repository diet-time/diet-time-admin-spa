...
import { Alert, Autocomplete, Box, Button, Card, CardContent, Chip, CircularProgress, Divider, FormControlLabel, Grid, IconButton, MenuItem, Stack, Switch, Tab, Tabs, TextField, Typography } from '@mui/material';
...
import { nutritionFieldSx, nutritionVisuals } from '../components/nutritionVisuals';
...
function Nutrition({ register, nutrition, errors }: { register: Register; nutrition: MealFormValues['nutrition']; errors: Errors }) {
  const fields = ['calories', 'protein', 'carbohydrates', 'fat', 'saturatedFat', 'transFat', 'fibre', 'sugar', 'sodium', 'cholesterol'] as const;
  const highlights = ['calories', 'protein', 'carbohydrates', 'fat'] as const;

  return <Stack spacing={3}>
    <Box>
      <Typography variant="h2">Nutrition</Typography>
      <Typography color="text.secondary" mt={0.5}>Enter the nutritional values for one serving.</Typography>
    </Box>
    <Grid container spacing={2}>
      <Grid size={{ xs: 6, md: 3 }}>
        <TextField fullWidth type="number" label={nutritionVisuals.servingQuantity.label} {...register('nutrition.servingQuantity')} sx={nutritionFieldSx('servingQuantity')} />
      </Grid>
      <Grid size={{ xs: 6, md: 3 }}>
        <TextField fullWidth label={nutritionVisuals.servingUnit.label} {...register('nutrition.servingUnit')} sx={nutritionFieldSx('servingUnit')} />
      </Grid>
      {fields.map((field) => <Grid key={field} size={{ xs: 6, md: 3 }}>
        <TextField
          fullWidth
          type="number"
          label={nutritionVisuals[field].label}
          {...register(`nutrition.${field}`)}
          error={!!errors.nutrition?.[field]}
          helperText={errors.nutrition?.[field]?.message}
          sx={nutritionFieldSx(field)}
        />
      </Grid>)}
    </Grid>
    <Divider />
    <Grid container spacing={1.25}>
      {highlights.map((metric) => <Grid key={metric} size={{ xs: 6, md: 3 }}>
        <NutritionMetricCard metric={metric} value={nutrition[metric]} />
      </Grid>)}
    </Grid>
  </Stack>;
}
function Review({ values, errors, onPublish, isPublishing, publishError }: { values: MealFormValues; errors: Errors; onPublish: () => void; isPublishing: boolean; publishError: boolean }) { const errorCount = Object.keys(errors).length; return <Stack spacing={2}><Typography variant="h2">Review & publish</Typography><Alert severity={errorCount ? 'error' : 'success'}>{errorCount ? `${errorCount} section(s) need attention before publishing.` : 'Core meal validation is complete.'}</Alert>{publishError && <Alert severity="error">The meal was saved, but it could not be published. Please try again.</Alert>}<Typography><strong>SKU:</strong> {values.sku || 'Missing'}</Typography><Typography><strong>English name:</strong> {values.translations.en.name || 'Missing'}</Typography><Typography><strong>Arabic translation:</strong> {values.translations.ar?.name || 'Missing — publishing policy may block this'}</Typography><Typography><strong>Availability:</strong> {values.availability.mode}</Typography><Button type="button" variant="contained" disabled={errorCount > 0 || isPublishing} onClick={onPublish}>{isPublishing ? 'Publishing…' : 'Publish meal'}</Button></Stack>; }
