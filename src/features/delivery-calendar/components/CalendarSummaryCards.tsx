import { CalendarMonthOutlined, LocalShippingOutlined, PeopleOutline, SwapHorizOutlined, TrendingUp } from '@mui/icons-material';
...
export function CalendarSummaryCards({ days }: { days: DeliveryCalendarDay[] }) {
  ...
  const values = [
    ...
    { label: 'Active customers', value: Math.max(0, ...days.map((day) => day.totalCustomers)), detail: peakDay ? `Peak volume: ${peakDay.totalDeliveries} orders` : 'No orders scheduled', icon: <PeopleOutline />, tone: '#2C6E9E', tint: '#EAF3F9' },
    { label: 'Menu overrides', value: days.reduce((sum, day) => sum + day.overrideCount, 0), detail: `${days.filter((day) => day.overrideCount > 0).length} dates affected`, icon: <SwapHorizOutlined />, tone: colors.orange, tint: '#FFF0EC' },
    { label: 'Holidays / closures', value: days.filter((day) => day.operationalStatus === 'CLOSED').length, detail: `${activeDays.length} operating days`, icon: <CalendarMonthOutlined />, tone: colors.mocca, tint: '#F5EEE9' },
  ];
