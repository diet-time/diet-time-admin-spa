...
export function CalendarMonthGrid({ days, view, onSelect }: { days: DeliveryCalendarDay[]; view: DeliveryView; onSelect: (date: string) => void }) {
  ...
  return <Box sx={{ overflowX: 'auto' }}><Box sx={{ minWidth: 840 }}><Grid container columns={7}>{weekdays.map((weekday) => <Grid size={1} key={weekday}><Typography variant="caption" fontWeight={800} color="text.secondary" display="block" px={1.5} py={1.25} borderBottom={1} borderColor="divider">{weekday}</Typography></Grid>)}{Array.from({ length: blanks }, (_, index) => <Grid size={1} key={`blank-${index}`} sx={{ bgcolor: '#F8FAF9', borderBottom: 1, borderInlineEnd: 1, borderColor: 'divider' }} />)}{visible.map((day) => <Grid size={1} key={day.date}><CalendarDayCell day={day} maxDeliveries={maxDeliveries} onSelect={onSelect} /></Grid>)}</Grid></Box></Box>;
}
